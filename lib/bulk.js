      ...
      for (const message of pendingMessages) {
        try {
          ...
          const cooldownCheck = await pool.query(
            `SELECT 1 FROM bulk_messages WHERE recipient_phone = ANY($1) AND status = 'sent' AND sent_at > NOW() - INTERVAL '24 hours' LIMIT 1`,
            [phoneKeys(message.recipient_phone)]
          );
          ...
          if (!bulkResult.ok) {
            if (bulkResult.reason === 'SPEND_CAP_EXCEEDED') {
              ...
              await pool.query(
                `UPDATE bulk_messages SET status = 'paused',
          ...
          if (bulkResult.sid) {
            await pool.query('UPDATE bulk_messages SET twilio_sid = $1 WHERE id = $2', [bulkResult.sid, message.id]);
          }
