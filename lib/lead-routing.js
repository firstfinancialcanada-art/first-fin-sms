...
// Sarah's leads never reached any of this.
//
// routeLead was wired to exactly one caller — lead-intake.js, the email/ADF
// path — while the channel that actually carries the volume is Sarah's SMS.
// A customer who texted in and booked an appointment produced an appointment
// row and a text to the managers, and nothing else: no CRM entry unless
// somebody later ran the manual "import from Sarah" screen, and therefore no
// assignment and no rep alert. The round-robin engine was running on the
// smallest channel in the building.
//
// This upserts the CRM row for a Sarah lead, routes it, and hands back who it
// landed on so the caller can text them. Keyed on phone within the tenant,
// which is how desk_crm is deduped elsewhere.
async function captureAndRouteSarahLead({ tenantId, userId, phone, name, vehicleType, budget, detail, source = 'SARAH' }) {
  if (!tenantId || !phone) return { captured: false, reason: 'missing_tenant_or_phone' };
  let crmId = null, created = false;
  try {
    const existing = await pool.query(
      `SELECT id, assigned_rep_id FROM desk_crm
    ...
    if (existing.rows.length) {
      ...
      // Already owned by a rep — don't reshuffle an in-flight customer just
      // because they texted again.
      if (existing.rows[0].assigned_rep_id) {
        return { captured: true, crmId, created: false, assigned: false, reason: 'already_assigned',
                 repId: existing.rows[0].assigned_rep_id };
      }
      // Fill gaps without overwriting anything a human has already set.
      await pool.query(
        `UPDATE desk_crm
            SET name             = COALESCE(NULLIF(name,''), $2),
                vehicle_interest = COALESCE(NULLIF(vehicle_interest,''), $3),
                budget_range     = COALESCE(NULLIF(budget_range,''), $4),
                last_contact     = NOW(),
                updated_at       = NOW()
          WHERE id = $1`,
        [crmId, name || '', vehicleType || '', budget || '']
      );
    } else {
      const ins = await pool.query(
        `INSERT INTO desk_crm (user_id, tenant_id, assigned_rep_id, name, phone,
                               vehicle_interest, budget_range, status, source, last_contact)
         VALUES ($1, $2, NULL, $3, $4, $5, $6, 'Lead', $7, NOW())
         RETURNING id`,
        [userId || null, tenantId, name || '', phone, vehicleType || '', budget || '', source]
      );
      crmId = ins.rows[0].id;
      created = true;
    }
    // Free text from the discovery step — what they said they were after, in
    // their words. Best-effort: the column may not exist on older tenants.
    if (detail) {
      await pool.query(
        `UPDATE desk_crm SET notes = COALESCE(NULLIF(notes,'') || E'\\n', '') || $2 WHERE id = $1`,
        [crmId, `Sarah: "${String(detail).slice(0, 200)}"`]
      ).catch(() => {});
    }
  } catch (e) {
    console.error('[lead-routing] sarah capture failed:', e.message);
    return { captured: false, reason: 'db_error' };
  }

  let routed = { assigned: false };
  try {
    routed = await routeLead({ tenantId, crmEntryId: crmId, source });
  } catch (e) {
    console.error('[lead-routing] sarah route failed:', e.message);
  }
  return { captured: true, crmId, created, ...routed };
}

module.exports = {
  captureAndRouteSarahLead,
  init,
  listRules,
  listActiveReps,
  createRule,
