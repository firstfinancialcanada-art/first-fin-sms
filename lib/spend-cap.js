...
const { TENANT_CAPS, TIER_CAPS, TWILIO_COST_ESTIMATE, EXEMPT_EMAILS } = require('./constants');
...
// ── Who pays ────────────────────────────────────────────────────────────
// The $18.50 is one allowance per DEALERSHIP, but tenant_usage is keyed on
// user_id, so every seat was getting its own: a seven-seat Gold tenant had
// seven separate allowances. Every read and write of the wallet now resolves
// to the tenant's OWNER, so the seats share one balance. Latent until now
// only because every tenant is still single-seat.
//
// Cached briefly: this runs on every outbound text, and tenant ownership
// changes about never.
const _billingIdCache = new Map();          // userId -> { id, at }
const BILLING_ID_TTL_MS = 5 * 60 * 1000;

async function billingUserId(userId) {
  const hit = _billingIdCache.get(userId);
  if (hit && (Date.now() - hit.at) < BILLING_ID_TTL_MS) return hit.id;
  let id = userId;
  try {
    const { rows } = await pool.query(
      `SELECT t.owner_user_id
         FROM desk_members m JOIN desk_tenants t ON t.id = m.tenant_id
        WHERE m.user_id = $1 AND m.active = TRUE
        ORDER BY m.id ASC LIMIT 1`,
      [userId]
    );
    if (rows[0] && rows[0].owner_user_id) id = rows[0].owner_user_id;
  } catch (e) {
    // Fall back to the caller's own row — worst case they get their own
    // allowance, which is exactly the old behaviour, never a free pass.
    console.warn('billingUserId lookup failed, using caller:', e.message);
  }
  _billingIdCache.set(userId, { id, at: Date.now() });
  return id;
}

async function isExempt(userId) {
  try {
    const payer = await billingUserId(userId);
    const { rows } = await pool.query('SELECT email FROM desk_users WHERE id = $1', [payer]);
...
async function getUsage(userId) {
  const payer  = await billingUserId(userId);
  const exempt = await isExempt(payer);
  await ensureUsageRow(payer);
  const { rows } = await pool.query(
...
async function recordSpend(userId, kind, cents) {
  if (cents <= 0) return;
  const payer = await billingUserId(userId);
  if (await isExempt(payer)) return;
  await ensureUsageRow(payer);
  ...
  await pool.query(
    `UPDATE tenant_usage
        SET ${field}             = ${field} + $2,
            ...
            last_updated          = NOW()
      WHERE user_id = $1`,
    [payer, cents, TENANT_CAPS.smsVoiceCombinedCents]
  );
}

// ── Row-count caps (inventory, CRM) ─────────────────────────────────────
// Which tier this user's tenant is on, and therefore which caps apply.
// Also returns the tenant id, because inventory and CRM are shared across a
// tenant: counting only the caller's own rows gave a five-rep Gold tenant
// five separate allowances, when the cap is meant to be per tenant.
async function tenantCaps(userId) {
  let tenantId = null, tier = 'single';
  try {
    const { rows } = await pool.query(
      `SELECT t.id, t.tier
         FROM desk_members m JOIN desk_tenants t ON t.id = m.tenant_id
    ...
    if (rows[0]) { tenantId = rows[0].id; tier = rows[0].tier || 'single'; }
  } catch (e) {
    console.warn('tenantCaps lookup failed, using defaults:', e.message);
  }
  const caps = TIER_CAPS[tier] || { inventory: TENANT_CAPS.inventoryMax, crm: TENANT_CAPS.crmMax };
  return { tenantId, tier, caps };
}

async function countFor(table, tenantId, userId) {
  const { rows } = tenantId
    ? await pool.query(`SELECT COUNT(*)::int AS c FROM ${table} WHERE tenant_id = $1`, [tenantId])
    : await pool.query(`SELECT COUNT(*)::int AS c FROM ${table} WHERE user_id = $1 AND tenant_id IS NULL`, [userId]);
  return rows[0]?.c || 0;
}

async function checkInventoryCap(userId) {
  const { tenantId, tier, caps } = await tenantCaps(userId);
  if (await isExempt(userId)) {
    return { ok: true, count: 0, cap: caps.inventory, tier, exempt: true };
  }
  const count = await countFor('desk_inventory', tenantId, userId);
  const cap = caps.inventory;
  return {
    ok: count < cap,
    count, cap, tier,
...
async function checkCrmCap(userId, addingCount = 1) {
  const { tenantId, tier, caps } = await tenantCaps(userId);
  if (await isExempt(userId)) {
    return { ok: true, count: 0, cap: caps.crm, tier, exempt: true };
  }
  const count = await countFor('desk_crm', tenantId, userId);
  const cap = caps.crm;
  return {
...
async function reconcileSpend(sid, actualCents) {
  ...
  try {
    ...
    if (delta !== 0) {
      ...
      // The event records WHO sent it, which is what you want in an audit
      // trail, but the correction belongs on the wallet that was charged.
      const payer = await billingUserId(row.user_id);
      await pool.query(
        `UPDATE tenant_usage
