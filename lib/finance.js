...
function calculateDeal(params) {
  ...
  // ── Reserve (two parallel calcs so dealers see both) ──────────────────
  const rateSpread = Math.max(0, contractRate - buyRate);
  // Flat formula — matches quick-quote conventions some lenders publish.
  // Treats balance as constant over term, so overstates vs. real payouts.
  const spreadReserve = rateSpread > 0
    ? Math.round((rateSpread / 100 / 12) * financed * term * (bankSplit / 100))
    : 0;
  // Amortization-accurate — actual incremental interest from the spread
  // over the declining balance, then dealer share. Matches what the lender
  // actually pays out on most real Canadian dealer-participation agreements.
  let spreadReserveActual = 0;
  if (rateSpread > 0 && financed > 0 && term > 0) {
    const contractMonthlyPmt = BPMT(contractRate, term, financed, 'monthly');
    const buyMonthlyPmt      = BPMT(buyRate,      term, financed, 'monthly');
    spreadReserveActual = Math.round(((contractMonthlyPmt - buyMonthlyPmt) * term) * (bankSplit / 100));
  }
  ...
  const backEnd = {
    ...
    reserve: { spread: round2(rateSpread), amount: spreadReserve, amountActual: spreadReserveActual },
  ...
  return {
    ...
    // Reserve (two views: flat quick-quote + amortization-accurate payout)
    rateSpread: round2(rateSpread), spreadReserve, spreadReserveActual,
    // Structured profit breakdown
    frontEnd, backEnd, dealTotal,
    // F&I product cost in deal (total price of all products)
    fiProductTotal: round2(vscPrice + gapPrice + twPrice + waPrice),
    fiProductInFinanced: round2(vscPrice + gapPrice + twPrice + waPrice), // amount added to loan for products
    // Legacy flat fields (backwards compatible)
