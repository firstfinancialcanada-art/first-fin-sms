...
const pool = new Pool({
  ...
  ssl: process.env.DB_SSL_REJECT_UNAUTHORIZED === 'true'
    ? { rejectUnauthorized: true }    // verified TLS — set when using managed Postgres outside Railway
    : { rejectUnauthorized: false },  // Railway internal — self-signed cert, safe within private network
  max: 20
});
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      if (conv.status !== 'active') {
        ...
        console.log('💬 Reactivated conversation:', phone, '(was:', conv.status + ')');
        conv.status = 'active';
      } else {
        await client.query(
