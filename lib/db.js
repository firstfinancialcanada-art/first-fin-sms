...
// settings_json->>'notifyPhone', so it was racing this conversion and
// losing — boot logs on 2026-09-19 still showed "notify init: operator
// does not exist: text ->> unknown" long after the migration itself had
// landed. The backfill never ran, which is why a tenant that set a notify
// number in the wizard could still have notify_phone NULL. Anything that
// reads settings_json at boot must await this.
...
async function updateConversation(conversationId, updates) {
  const ALLOWED_FIELDS = new Set([
    ...
    'mode', 'vehicle_make', 'vehicle_model', 'vehicle_year', 'vehicle_mileage',
