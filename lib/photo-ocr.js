...
function _ensureCache() {
  if (!_cacheReady) {
    ...
    `).catch(e => { console.error('❌ photo_sign_checks table:', e.message); });
  }
  return _cacheReady;
}
...
async function classifyVehiclePhotos(urls) {
  ...
  const cached = await _readCache(urls);
  const toScan = urls.filter(u => !cached.has(u));
  const scanned = [];
  ...
  // kept so the vehicle still has something to display.
