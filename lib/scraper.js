...
const { safeFetch } = require('./url-guard'); // SSRF-guarded fetch (SmartBuy API call)

// ── Text helpers (pure functions, no DOM) ────────────────────────────────
function clean(t) { return (t || '').replace(/\s+/g, ' ').trim(); }

function parsePrice(t) {
  // Payment and down-payment figures are not the vehicle price.
  const s = (t || '').replace(/,/g, '')
    .replace(/\$?\s*\d{3,6}\s*(?:\/\s*)?(?:down|bi-?weekly|biweekly|weekly|monthly|month|mo|per\s*week|oac)/gi, ' ');
  const m = s.match(/\$\s*(\d{4,6})(?!\d)/);
  if (m) return parseInt(m[1]);
  // Bare numbers used to count when >= 1000, so a "Call for price" card
  // returned the model year: a 2019 truck imported at $2,019 and was postable
  // to Marketplace at that price. A bare number now counts only after a price
  // word, and never when it reads like a year. No price is the safe failure —
  // the poster already holds those cars back and says they're waiting on one.
  const m2 = s.match(/(?:price|asking|sale|cash)\D{0,12}(\d{4,6})(?!\d)/i);
  const v = m2 ? parseInt(m2[1]) : 0;
  return (v >= 2500 && !(v >= 1980 && v <= 2035)) ? v : 0;
}
...
// Map a make/model to a high-confidence body style, used as a tiebreaker
// when Hunt's "Category:" line is missing and the body-text fallback might
// match the wrong word from a sidebar listing. Returns '' when not sure —
// callers should keep their previous guess in that case. Heavily weighted
// toward Hunt's Chrysler/Dodge/Jeep/RAM lineup but covers the common
// cross-brand trade-ins they get too.
function bodyStyleFromModel(make, model) {
  const m = ((make || '') + ' ' + (model || '')).toLowerCase().replace(/[_-]/g, ' ');
  if (!m.trim()) return '';
  // Convertus model names often state the body themselves ("RS5 Coupe",
  // "Civic Si Sedan", "Sierra 1500 Crew Cab") — trust that over the lists.
  if (/\b(crew cab|supercrew|supercab|quad cab|double cab|regular cab|super duty|king cab|access cab)\b/.test(m)) return 'Pickup';
  if (/\bsedan\b/.test(m))                 return 'Sedan';
  if (/\bcoupe\b/.test(m))                 return 'Coupe';
  if (/\b(hatchback|hatch)\b/.test(m))     return 'Hatchback';
  if (/\b(wagon|sportwagen|allroad)\b/.test(m)) return 'Wagon';
  if (/\b(convertible|cabriolet|roadster|spyder)\b/.test(m)) return 'Convertible';
  if (/\b(minivan|cargo van|passenger van)\b/.test(m)) return 'Van';
  // Pickups
...
function addPhoto(src, photos, seen, altText, softRejected) {
  ...
  photos.push(upsizePhotoUrl(cleaned));
}
...
function parseVdpDetailHtml(html, url) {
  ...
  // \s* prefix lets the lookahead skip whitespace between value and next
  // label (cheerio sometimes inserts spaces, browsers insert newlines, etc).
  // Char class allows /, comma, &, hyphen so categories like
  // "SUVs / Crossovers" are captured cleanly instead of failing at the slash.
  ...
  const categoryRaw = specVal(new RegExp('Category\\s*:\\s*(' + VALCHARS + ')' + NEXT_LABEL, 'i'));
  const extColorRaw = specVal(new RegExp('Exterior\\s*Colou?r\\s*:\\s*(' + VALCHARS + ')' + NEXT_LABEL, 'i'));
  const intColorRaw = specVal(new RegExp('Interior\\s*Colou?r\\s*:\\s*(' + VALCHARS + ')' + NEXT_LABEL, 'i'));
  ...
  // Model-name override: when Hunt's spec block doesn't have a Category line
  // (older / brand-placeholder-only inventory), the fallback grep above can
  // pick up "Sedan" from a sidebar listing like "Honda Civic Sedan" on a
  // Jeep Grand Cherokee VDP. If we recognize the model as something with a
  // known body style, trust that over the body-text grep. Heavily weighted
  // toward Hunt's Chrysler/Dodge/Jeep/RAM lineup.
  const _modelGuess = bodyStyleFromModel(slug.make || '', slug.model || '');
  ...
  const cfx = parseCarfaxFromHtml(html);
  if (cfx.badges.length) result.carfax_badges = cfx.badges.join(', ');
  if (cfx.url) result.carfax_url = cfx.url;

  // schema.org Car JSON-LD. Fox Dealer (Automaxx, House of Cars' old skin)
  // puts price, make/model, drivetrain and km only here — pre-fix Automaxx
  // VDPs came back price 0, make "Mercedes" / model "Benz" — so on Fox sites
  // it wins outright. Elsewhere JSON-LD is often worse than the DOM (Hunt's
  // has 3 photos vs 26, marketing text in `name`, wrong drivetrain), so it
  // only fills fields the DOM left empty, and never trim/type/make/model/stock.
  const ld = readVehicleJsonLd($);
  if (ld) {
    const isFox = /static\.foxdealer\.com|foxdealer\.com\/wp-content/i.test(html);
    const GAP_FILL = ['vin', 'year', 'price', 'mileage', 'color', 'transmission', 'drive_train', 'fuel_type'];
    for (const [k, v] of Object.entries(ld)) {
      if (!v) continue;
      if (isFox) {
        // DOM spec rows are more specific ("9-Speed Automatic" vs "Automatic")
        if (k === 'transmission' && result[k] && result[k].toLowerCase().includes(v.toLowerCase())) continue;
        result[k] = v; continue;
      }
      if (k === '_photos') { if (!result._photos.length) result._photos = v; continue; }
      if (GAP_FILL.includes(k) && !result[k]) result[k] = v;
    }
  }
  return result;
}

// ── schema.org Car / Vehicle JSON-LD ─────────────────────────────────────
function readVehicleJsonLd($) {
  let car = null;
  $('script[type="application/ld+json"]').each((i, el) => {
    ...
    car = nodes.find(n => n && /\b(Car|Vehicle|MotorizedBicycle)\b/.test([].concat(n['@type']).join(' '))) || null;
  });
  if (!car) return null;

  const name  = clean(car.name || '');
  const year  = parseInt(car.vehicleModelDate || car.modelDate || '') || 0;
  // JSON-LD brand is already correctly cased ("Mercedes-Benz", "Alfa Romeo"),
  // so only upcase the acronym makes — normMake() would flatten the hyphen
  // and give "Mercedes Benz", which then breaks the trim match below.
  const brandRaw = clean((car.brand && car.brand.name) || car.manufacturer || '');
  const make  = MAKE_UPCASE.has(brandRaw.toUpperCase()) ? brandRaw.toUpperCase() : brandRaw;
  const model = clean(typeof car.model === 'string' ? car.model : (car.model && car.model.name) || '');

  // Trim: name is "YEAR MAKE MODEL TRIM 4dr Sdn ..." — take what follows the
  // model up to the first body/door descriptor.
  let trim = '';
  if (name && make && model) {
    const esc = s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const m = name.match(new RegExp('^\\d{4}\\s+' + esc(make) + '\\s+' + esc(model) + '\\s+(.*)$', 'i'));
    if (m) {
      const rest = clean(m[1]).split(/\s+/);
      // Fox Dealer states the trim twice back to back — "SLT+ SLT+ 3500 Super
      // High Roof…", "Big Bend Big Bend 4x4…" — so the doubled run IS the
      // trim, and it ends where the repeat ends. Longest repeat wins so
      // "Big Bend Big Bend" beats a chance single-word match.
      for (let n = 4; n >= 1; n--) {
        if (rest.length < n * 2) continue;
        const a = rest.slice(0, n).join(' '), b = rest.slice(n, n * 2).join(' ');
        if (a.toLowerCase() === b.toLowerCase()) { trim = a; break; }
      }
      // No repeat (e.g. "CLA 250 4dr Sdn CLA 250 4MATIC") — cut at the first
      // body/door/engine descriptor instead.
      if (!trim) trim = clean(rest.join(' ').split(/\s+(?:\d\s*dr|\d-door|Sdn|Sedan|Cpe|Coupe|Hatchback|HB|Wagon|Conv|Convertible|Crew|Quad|Reg|Double|Ext|SuperCrew|SuperCab|Cab|4x4|4x2|AWD|FWD|RWD|4WD|2WD|Intercooled|Turbo|Gas|Regular|Premium)\b/i)[0]);
      // Drop a word that already appeared: Fox also writes the trim as
      // "Preferred 2.0L Preferred", where the repeat isn't adjacent so the
      // run check above can't see it.
      const seenWord = new Set();
      trim = trim.split(' ').filter(w => {
        const k = w.toLowerCase();
        if (seenWord.has(k)) return false;
        seenWord.add(k); return true;
      }).join(' ');
    }
  }

  const offer = [].concat(car.offers || [])[0] || {};
  const price = parseInt(String(offer.price || '').replace(/[^\d.]/g, '')) || 0;

  let mileage = 0;
  const odo = car.mileageFromOdometer;
  if (odo) {
    const v = parseInt(String(typeof odo === 'object' ? odo.value : odo).replace(/[^\d]/g, '')) || 0;
    mileage = (odo.unitCode === 'SMI') ? Math.round(v * 1.609344) : v;
  }

  const DRIVE = { AllWheelDriveConfiguration: 'AWD', FourWheelDriveConfiguration: '4WD', FrontWheelDriveConfiguration: 'FWD', RearWheelDriveConfiguration: 'RWD' };
  const driveKey = String(car.driveWheelConfiguration || '').split('/').pop();

  // vehicleConfiguration is Fox Dealer's own body class ("Cargo Vans",
  // "4WD Sport Utility Vehicles", "4-door Compact Passenger Car").
  const tokens = ' ' + name + ' ' + (car.bodyType || '') + ' ' + (car.vehicleConfiguration || '') + ' ';
  let type = '';
  if (/\b(Crew|Quad|Double|Reg(?:ular)?|Ext(?:ended)?|Super)\s*Cab\b|\bSuperCrew\b|\bPickup\b/i.test(tokens)) type = 'Pickup';
  else if (/\bSUV\b|Sport Utility/i.test(tokens)) type = 'SUV';
  else if (/\b(Sdn|Sedan)\b/i.test(tokens))       type = 'Sedan';
  else if (/\b(Cpe|Coupe)\b/i.test(tokens))       type = 'Coupe';
  else if (/\b(Hatchback|HB)\b/i.test(tokens))    type = 'Hatchback';
  else if (/\bWagon\b/i.test(tokens))             type = 'Wagon';
  else if (/\b(Conv|Convertible)\b/i.test(tokens)) type = 'Convertible';
  else if (/\b(Van|Minivan)\b/i.test(tokens))     type = 'Van';

  // HomeNet stock_images are manufacturer press shots, not this unit. Keep
  // them so the car isn't blank on the desk (2026-09-17: 10 Automaxx units
  // are shot-pending and showed as 0 photos), but they must never reach
  // Marketplace as the actual vehicle — the FB autofill path filters them
  // out by this same /stock_images/ marker.
  const photos = [].concat(car.image || [])
    .map(im => typeof im === 'string' ? im : (im && (im.contentUrl || im.url)) || '')
    .filter(Boolean);

  return {
    vin:          clean(car.vehicleIdentificationNumber || ''),
    stock:        clean(car.sku || '').toUpperCase(),
    year:         (year >= 1980 && year <= 2035) ? year : 0,
    make, model, trim, type,
    price:        price >= 1000 ? price : 0,
    mileage:      (mileage > 0 && mileage < 1000000) ? mileage : 0,
    color:        parseColor(car.color || ''),
    transmission: clean(car.vehicleTransmission || ''),
    drive_train:  DRIVE[driveKey] || '',
    fuel_type:    clean((car.vehicleEngine && car.vehicleEngine.fuelType) || car.fuelType || '').replace(/\s*Fuel$/i, ''),
    _photos:      car.image ? photos.map(upsizePhotoUrl) : null
  };
}

// ── Carfax badges ────────────────────────────────────────────────────────
// "One owner, no accidents" is the line that makes a Marketplace buyer
// message you, so it's worth carrying into the listing. Fox Dealer sites
// (Automaxx) render the badge server-side as the image name — Logo_NoBadges,
// Logo_AccidentFree, Logo_OneOwner_AccidentFree — plus a report link.
// Convertus (South Trail) injects it from badgingapi.carfax.ca in the
// browser instead, and that API 401s outside the dealer's page, so the
// extension collects those during the scan.
const CARFAX_LABELS = {
  oneowner:        'One Owner',
  // Carfax names the badge AccidentFree but displays it as "No Reported
  // Accidents" on the dealer sites — match what a buyer would have seen.
...
function carfaxLabel(name) {
  return CARFAX_LABELS[String(name).toLowerCase().replace(/[^a-z]/g, '')] || '';
}
function parseCarfaxFromHtml(html) {
  const badges = [];
  const m = html.match(/badging\/v\d\/[a-z]{2}\/(?:Logo_)?([A-Za-z_]+)\.svg/);
  if (m && !/^NoBadges$/i.test(m[1])) {
    for (const part of m[1].split('_')) {
      const label = carfaxLabel(part);
      if (label && !badges.includes(label)) badges.push(label);
    }
  }
  const url = (html.match(/https:\/\/vhr\.carfax\.ca\/\?id=[^"'\s\\]+/) || [])[0] || '';
  return { badges, url };
}

// HomeNet serves every photo at several sizes off the same GUID; the 640x480
// dealers embed is soft on a phone. 1280x960 verified available 2026-09-16.
function upsizePhotoUrl(src) {
  return String(src).replace(/(homenetiol\.com)\/640x480\//i, '$1/1280x960/');
}
...
async function fetchSmartBuyInventory(html) {
  try {
    ...
    // apiUrl host is regex-extracted from caller-supplied HTML — safeFetch
    // confirms it resolves to a public IP so crafted HTML can't redirect the
    // server at internal targets. Security audit 2026-06-18.
    const r = await safeFetch(apiUrl, {
    ...
    return rows
      ...
      .map(v => {
        ...
        // SmartBuy's feed is dealer-entered, so casing is whatever was typed
        // ("DODGE", "WHITE") and the trim field carries the drivetrain
        // ("GT AWD/All-Wheel Drive", "SE 4x2"). Marketplace listings show
        // these verbatim, so normalize here: split the drivetrain into its
        // own field and title-case make/colour to match every other source.
        const make  = normMake(v.make || '');
        // Models come in shouting ("JETTA HIGHLINE", "QASHQAI"). Title-case
        // the long words but leave short all-caps badges alone — GTI, XE,
        // RAV4 are not "Gti", "Xe", "Rav4".
        // Plain words that show up shouting in trim-style model names
        // ("1500 BIG HORN") and are short enough to survive the badge rule.
        const SHOUTY = new Set(['BIG','HORN','KING','RANCH','CREW','CAB','MAX','PLUS','LONG','BOX','SPORT','BASE','DUAL','HIGH','QUAD']);
        const model = clean(v.model || '').split(' ').map(w => {
          const isLongAllCaps = /^[A-Z0-9-]{5,}$/.test(w);
          return (SHOUTY.has(w) || isLongAllCaps) ? titleWord(w.toLowerCase()) : w;
        }).join(' ');
        const trimRaw = clean(v.trim || '');
        const DRIVE_RE = /\b(AWD|FWD|RWD|4WD|2WD|4x4|4x2|All-Wheel Drive|Front-Wheel Drive|Rear-Wheel Drive|Four-Wheel Drive|4-Wheel Drive|2-Wheel Drive)\b/i;
        const driveM = trimRaw.match(DRIVE_RE);
        const DRIVE_CANON = { 'all-wheel drive': 'AWD', 'front-wheel drive': 'FWD', 'rear-wheel drive': 'RWD', 'four-wheel drive': '4WD', '4-wheel drive': '4WD', '2-wheel drive': '2WD', '4x4': '4WD', '4x2': '2WD' };
        const driveRaw = driveM ? driveM[1] : '';
        const drive = driveRaw ? (DRIVE_CANON[driveRaw.toLowerCase()] || driveRaw.toUpperCase()) : '';
        const trim = clean(trimRaw
          .replace(new RegExp(DRIVE_RE.source, 'gi'), ' ')
          .replace(/\s*\/\s*/g, ' '))
          .split(' ').map(w => (/^[A-Z0-9.+-]{1,4}$/.test(w) && !SHOUTY.has(w)) ? w : titleWord(w.toLowerCase())).join(' ');
        return {
          ...
          color:     parseColor(v.exterior_color || '') || titleWord((v.exterior_color || '').toLowerCase()),
          // The feed has no body style, so every unit imported as "Used" and
          // a Marketplace listing needs a real one. Derive it from the model.
          type:      bodyStyleFromModel(make, model) || 'Used',
          condition: 'Used',
          carfax:    0,
          book_value: 0,
          ...
          // Wholesale: these are AMVIC-dealer-only prices. The FB Poster
          // refuses to post a unit carrying this source until a per-unit
          // retail price is set, so dealer cost never reaches Marketplace.
          _source:   'smartbuyauto.ca',
          _wholesale: true
        };
      });
  } catch (e) {
...
module.exports = { scrapePageHtml, parseVdpDetailHtml, parseFromSlug, bodyStyleFromModel, VDP_LINK_RE, D2C_VDP_RE };
