...
function readVehicleJsonLd($) {
  ...
  const make  = clean((car.brand && car.brand.name) || car.manufacturer || '');
  ...
  if (name && make && model) {
    ...
    // Fox Dealer repeats the trim ("SEL SEL 4WD") — collapse the doubled run.
    if (m) trim = clean(m[1].split(/\s+(?:\d\s*dr|\d-door|Sdn|Sedan|Cpe|Coupe|Hatchback|HB|Wagon|Conv|Convertible|Crew|Quad|Reg|Double|Ext|SuperCrew|SuperCab|Cab|4x4|4x2|AWD|FWD|RWD|4WD|2WD|Intercooled|Turbo|Gas|Regular|Premium)\b/i)[0]);
    const w = trim.split(' '), half = w.length / 2;
    if (w.length > 1 && w.length % 2 === 0 && w.slice(0, half).join(' ').toLowerCase() === w.slice(half).join(' ').toLowerCase()) trim = w.slice(0, half).join(' ');
  }
  ...
  const tokens = ' ' + name + ' ' + (car.bodyType || '') + ' ';
  let type = '';
