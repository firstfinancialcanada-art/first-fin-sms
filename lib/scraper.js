...
function addPhoto(src, photos, seen, altText, softRejected) {
  ...
  if (/homenetiol\.com\/stock_images\//i.test(src)) return; // manufacturer press shot, not the unit
...
function readVehicleJsonLd($) {
  ...
  // HomeNet stock_images are manufacturer press shots, not this unit — posting
  // them to Marketplace as the actual car gets the account flagged. Drop them;
  // a unit left with zero photos waits for real ones.
  const photos = [].concat(car.image || [])
    .map(im => typeof im === 'string' ? im : (im && (im.contentUrl || im.url)) || '')
    .filter(u => u && !/\/stock_images\//i.test(u));
