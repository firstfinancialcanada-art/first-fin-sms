...
function extractPhotos($, url) {
  ...
  // dealer-photo CDN URLs that the gallery container selector might have
  // missed. Hunt Chrysler exposes ~26 unique d2cmedia photo positions per
  // VDP but the gallery container only renders ~20 of them — the rest
  // sit in non-gallery DOM (preload links, hidden carousel slots, etc).
  // Bumped guard from `< 3` to `< 30` so this strategy fills up to the
  // cap even when Strategy 1 already returned a healthy-but-incomplete
  // set. Caught 2026-04-27 — Hunt RAMs showed 📷20 in scan log even
  // though the dealer's photo viewer reached "Photo #27".
  if (photos.length < 30) {
    $('img').each((i, img) => {
      const $img = $(img);
