...
function addPhoto(src, photos, seen, altText) {
...
function extractPhotos($, url) {
  ...
  if (galleryEl) {
    galleryEl.find('img').each((i, img) => {
      const $img = $(img);
      addPhoto(getBestSrc($img), photos, seen, $img.attr('alt') || $img.attr('title') || '');
    });
  }
  ...
  if (photos.length < 3) {
    $('img').each((i, img) => {
      ...
      if (/homenet|dealerphoto|dealerphotos|vehiclephoto|d2cmedia|imagescdn|autotradercdn|getedealer|pictures\.dealer\.com|cdn.*\/(640|800|1024|1280)/i.test(src)) {
        addPhoto(src, photos, seen, $img.attr('alt') || $img.attr('title') || '');
      }
    });
  }
  ...
  // Phase 6: bumped from 10 -> 25 to match the extension's deep-scan cap.
    ...
    return rows
      ...
      .map(v => {
        ...
        return {
          ...
          _photos:   v.image_urls.slice(0, 20),
