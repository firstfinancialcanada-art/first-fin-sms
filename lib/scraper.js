...
function extractPhotos($, url) {
  const photos = [];
  const seen = new Set();
  // Fallback pool — soft-rejected images (brand logos, SVG placeholders).
  // If all extraction strategies finish with photos[] empty (dealer hasn't
  // uploaded real photos for this unit), we fall back to these so the
  // vehicle isn't shown as photo-less in inventory / FB Poster.
  const softRejected = [];
  ...
  // Fallback: dealer only uploaded a brand logo / SVG placeholder for this
  // unit (common on older inventory). Better to show that than nothing.
  if (photos.length === 0 && softRejected.length > 0) {
  ...
  if (!isVdpDetail) {
    ...
    if (huntCards.length >= 2) {
      ...
      huntCards.each((i, card) => {
        ...
        const stock = $c.attr('data-carid') || `H${String(i+1).padStart(4,'0')}`;
        cardVehicles.push({
          stock, vin: '',
