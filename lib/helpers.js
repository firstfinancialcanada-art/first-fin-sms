...
// ── Business hours ────────────────────────────────────────────────
// DEPRECATED — superseded by lib/hours.js. This reads BUSINESS_HOURS_* from
// the environment, which is process-wide, so every tenant got one store's
// hours in one store's timezone. It also hard-coded Monday–Friday, which told
// every dealer's Saturday callers they were closed. Nothing calls it now;
// kept only so an old import doesn't crash. Use hours.isOpenNow(tenant
// settings) instead.
