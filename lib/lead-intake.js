...
const { parseAdfXml, parsePlainTextLead, leadToCrmRow } = require('./adf-parser');
...
async function processMessage(parsedMail, rawSource = 'imap') {
  ...
  try {
    ...
    // Try the canonical ADF XML path first (Kijiji, AutoTrader, CarCostCanada,
    // TAQ all use this). If the email has no ADF — or the ADF didn't parse —
    // fall through to the plain-text fallback (CarGurus and similar providers
    // ship formatted plain-text emails instead of ADF). Either way, by the
    // time we're past this block, `parsed` is the normalized lead shape that
    // the content-fallback router and CRM insert both expect.
    const xml = extractAdfXml(parsedMail);
    let parsed = null;
    if (xml) parsed = parseAdfXml(xml);
    if (!parsed || !parsed.ok) {
      const body = parsedMail.text || (parsedMail.html ? String(parsedMail.html).replace(/<[^>]+>/g, ' ') : '');
      const ptr = parsePlainTextLead(body);
      if (ptr && ptr.ok) parsed = ptr;
    }
    ...
    // We have a tenant. Now require a parsed lead to actually create the
    // CRM row. parsed could have come from either parseAdfXml or
    // parsePlainTextLead at this point — both return the same shape.
    // Status 'no_adf' is retained for backward-compat with the replay/
    // dedup logic that uses it to identify reprocessable failures.
    if (!parsed || !parsed.ok) {
      summary.status = 'no_adf';
      summary.error  = parsed?.error || 'No ADF XML or plain-text lead found';
      await logIntake(summary);
      return summary;
    }
...
function startPolling() {
  ...
  // pollOnce() returns { status: 'error' } instead of throwing, and the
  // result used to be discarded — so an expired Gmail app password or a host
  // change stopped ADF lead email intake permanently and silently. Log it,
  // and shout once a run of failures looks like an outage rather than a blip.
  let _failStreak = 0;
  const runPoll = async () => {
    try {
      ...
      if (r && r.status === 'error') {
        _failStreak++;
        console.error(`❌ lead-intake poll failed (${_failStreak} in a row): ${r.error}`);
        if (_failStreak === 5) console.error('❌ lead-intake: 5 consecutive failures — email leads are NOT being collected');
      } else if (_failStreak) {
        console.log(`✅ lead-intake recovered after ${_failStreak} failure(s)`);
        _failStreak = 0;
      }
    } catch (e) {
      _failStreak++;
      console.error('❌ lead-intake poll threw:', e.message);
    }
  };
  // First poll fires after a short delay so startup logs are clean
  setTimeout(runPoll, 5000);
  _pollTimer = setInterval(runPoll, intervalSec * 1000);
}
