...
function seatsForTier(tier) {
  if (tier === 'platinum') return PLATINUM_DEFAULT_SEATS;
  if (tier === 'gold')     return GOLD_DEFAULT_SEATS;
  return SINGLE_DEFAULT_SEATS;
}
...
async function setTenantTier(tenantId, tier) {
  ...
  await pool.query(
    ...
    [tenantId, tier, seatsForTier(tier)]
  );
}

// Map a Stripe plan code (monthly/annual/gold_monthly/etc) to a tier.
function tierFromPlan(plan) {
  if (typeof plan !== 'string') return 'single';
  if (plan.startsWith('platinum')) return 'platinum';
  if (plan.startsWith('gold'))     return 'gold';
  return 'single';
}

// Idempotent: ensure a tenant + owner member row exists for this user.
// Used by the Stripe webhook (Phase 6e — fixes the gap where the old
// webhook only created desk_users, leaving brand-new buyers without a
// tenant context until next deploy ran the init() backfill) and by the
// admin "onboarding repair" endpoint to fix accounts created before this
// landed. If a tenant exists with a different tier, it's updated to match.
async function ensureOwnerTenant(userId, dealership, tier = 'single') {
  if (!VALID_TIERS.includes(tier)) tier = 'single';
  const seats = seatsForTier(tier);
  const existing = await pool.query(
    `SELECT id, tier, seats_allowed FROM desk_tenants WHERE owner_user_id = $1`,
    [userId]
  );
  let tenantId;
  ...
  if (existing.rows.length) {
    ...
    if (existing.rows[0].tier !== tier || existing.rows[0].seats_allowed < seats) {
      await pool.query(
        `UPDATE desk_tenants SET tier = $2, seats_allowed = $3 WHERE id = $1`,
        [tenantId, tier, seats]
      );
    ...
    const ins = await pool.query(
      ...
      [userId, dealership || '', tier, seats]
    );
    tenantId = ins.rows[0].id;
    createdTenant = true;
  }
  // Owner member row — UPSERT (re-activate if previously soft-removed).
  await pool.query(
    `INSERT INTO desk_members (tenant_id, user_id, role, crm_mode, active)
     VALUES ($1, $2, 'owner', 'team_read', TRUE)
     ON CONFLICT (tenant_id, user_id) DO UPDATE
       SET role = 'owner', active = TRUE`,
    [tenantId, userId]
  );
  return { tenantId, createdTenant, tier };
}
