...
const TENANT_CAPS = {
  smsVoiceCombinedCents: 1850, // $18.50 CAD allowance per tenant per month (all tiers)
  inventoryMax:          1000, // fallback when a tenant's tier is unknown
  crmMax:                1000, // fallback when a tenant's tier is unknown
  softWarnPct:           80,   // emit warning at 80% of cap (UI banner)
};

// Vehicle + CRM caps by tier. Every tier used to get 1000/1000.
//
// The April plan wanted Solo at 500, but Solo is NOT only small
// independents: South Trail runs 399 units on a single seat, and a
// franchise lot at 300-600 is ordinary. A 500 cap would have blocked
// their next inventory sync. Solo stays where it is and the upper tiers
// go up, which is also the honest way round — nobody loses anything they
// already had. Storage is not the constraint (a 1000-vehicle tenant costs
// fractions of a cent a month); this is tier shape, so change the numbers
// here when the pricing changes.
const TIER_CAPS = {
  single:   { inventory: 1000, crm: 1000 },
  gold:     { inventory: 2500, crm: 2500 },
  platinum: { inventory: 5000, crm: 5000 },
};

// How many Twilio numbers one tenant may hold. Numbers are bought on OUR
// master account and billed to us (~$1.15/mo each, forever), while
// provision-number only ever rewrites desk_users.twilio_number — it never
// releases the one being replaced. So repeated provisioning quietly
// accumulates numbers nobody references and everybody pays for; that is
// exactly how 424-789-7419 sat unused from February to September at $1.90 a
// month. These allow a changeover or two, not a runaway.
const TIER_NUMBER_CAPS = { single: 2, gold: 5, platinum: 15 };
...
// Inventory sources whose imported price is dealer-only (AMVIC wholesale).
// Those units must never list at the imported number: the FB Poster uses a
// per-unit retail_price instead. Kept server-side and sent as an
// is_wholesale flag, so the public poster code doesn't name the supplier.
const WHOLESALE_SOURCES = ['smartbuyauto.ca'];

module.exports = { EXEMPT_EMAILS, LENDER_FEES, TENANT_CAPS, TIER_CAPS, TIER_NUMBER_CAPS, TWILIO_COST_ESTIMATE, WHOLESALE_SOURCES };
